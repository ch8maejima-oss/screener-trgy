...
import type { TenbaggerSimPosition, TenbaggerSimState } from "@/lib/types";
import EquitySimChart from "./components/EquitySimChart";
import { tradingViewUrl } from "@/lib/tradingview";

export const metadata: Metadata = {
  title: "テンバガー候補スクリーニング フォワードシミュレーション | スクリーニング銘柄一覧",
  description:
    "テンバガー候補スクリーニングの通過銘柄を、あらかじめ固定した機械的な売買ルールで仮想売買した場合の、本日以降の成績記録です。実際の売買は行っていません。",
};

const EXIT_REASON_LABEL: Record<string, string> = {
  target: "利確（10倍到達・残り全量）",
  stopLoss: "損切り（-40%）",
  maxHold: "手仕舞い（保有上限＝5年）",
};

function loadState(): TenbaggerSimState | null {
  try {
    const p = path.join(process.cwd(), "data", "tenbagger-sim-state.json");
    const raw = fs.readFileSync(p, "utf-8");
    return JSON.parse(raw) as TenbaggerSimState;
  } catch {
    return null;
  }
}
...
function PositionsTable({ positions, showExit }: { positions: TenbaggerSimPosition[]; showExit: boolean }) {
      ...
      <table className="results__table">
        <thead>
          <tr>
            ...
            <th scope="col">1回目利確（3倍・1/3）</th>
            <th scope="col">2回目利確（6倍・1/3）</th>
            {showExit && (
              <>
                <th scope="col">最終決済日</th>
                <th scope="col" className="num">最終決済価格</th>
                <th scope="col">理由</th>
              </>
            )}
        ...
        <tbody>
          {positions.map((pos) => (
            <tr key={pos.id}>
              ...
              <td>
                {pos.partial1ExitPrice !== null
                  ? `${pos.partial1ExitDate}（${fmtPrice(pos.partial1ExitPrice)}）`
                  : "—"}
              </td>
              <td>
                {pos.partial2ExitPrice !== null
                  ? `${pos.partial2ExitDate}（${fmtPrice(pos.partial2ExitPrice)}）`
                  : "—"}
              </td>
              {showExit && (
                <>
    ...
    return (
      <div className="home">
        <section className="intro">
          <h1>テンバガー候補スクリーニング フォワードシミュレーション</h1>
    ...
    <div className="home equity-sim">
      <section className="intro">
        ...
        <p className="intro__lead">
          テンバガー候補スクリーニングの通過銘柄を買った場合、このスクリーニング条件は
          実際に機能するのか。{state.startDate}から、あらかじめ固定した機械的な売買
          ルールでの仮想売買を、勝敗を問わずすべて記録・追跡しています。
        </p>
      </section>
      ...
      <div className="equity-sim__notice">
        ...
        <ul>
          ...
          <li>判定は日次終値ベースで行っています（保有期間が最大5年に及ぶため）。</li>
          ...
          <li>
            買いの基準は、当社のテンバガー候補スクリーニングの出力を用いています。
            当該スクリーニング条件自体に前提や偏りがある場合、その影響は本シミュレーションの
            結果にも及びます。本記録は条件と実際の値動きの関係を検証する一つの試みであり、
            条件の正確性・優位性を保証するものではありません。
          </li>
          <li>
            テンバガー（10倍株）は実現するとしても数年〜10年単位の時間を要するとされており、
            本シミュレーションの結果が意味を持つまでには長期間を要します。短期間の成績のみで
            条件の有効性を判断しないでください。
          </li>
      ...
      <section className="equity-sim__rules">
        ...
        <ol>
          <li>
            <b>エントリー</b>：当日テンバガー候補スクリーニングの8/9条件以上を通過した
            銘柄を、当日終値で仮想的に建てます。
          </li>
          <li>
            <b>利確（1回目・1/3）</b>：株価が購入株価の3倍に到達した時点で、保有の1/3を
            手仕舞います。
          </li>
          <li>
            <b>利確（2回目・1/3）</b>：1回目の利確が済んだ状態で、株価が購入株価の6倍に
            到達した時点で、さらに1/3を手仕舞います。
          </li>
          <li>
            <b>利確（最終・残り全量）</b>：株価が購入株価の10倍に到達した時点で、その時点の
            残り数量をすべて手仕舞います（1回目・2回目を経ずに直接10倍に到達した場合も同様です）。
          </li>
          <li>
            <b>損切り</b>：終値が購入株価から40%以上下落した時点で、残り数量をすべて
            手仕舞います。
          </li>
          <li>
            <b>手仕舞い（保有上限）</b>：購入から5年（1,260営業日で近似）経過した時点で、
            達成状況にかかわらず残り数量をすべて手仕舞います。
          </li>
        </ol>
      </section>
