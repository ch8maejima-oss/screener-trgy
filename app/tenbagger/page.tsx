...
export const metadata: Metadata = {
  title: "テンバガー候補スクリーニング | スクリーニング銘柄一覧",
  description:
    "時価総額・売上高成長率・経常利益成長率・自己資本比率・上場からの年数・株価・出来高・PBR・増資の有無の9条件を、東証プライム市場およびスタンダード市場の全銘柄に機械的に適用した結果を表示します。株式会社トリロジー（近畿財務局長（金商）第372号）。",
};

// JSONから推論される型は各フィールドの実データに依存するため、
// 型定義側を正とみなして変換する。構造は build_tenbagger_site_data.py が保証する。
const screening = data as unknown as TenbaggerScreeningData;
    ...
    <div className="home">
      <section className="intro">
        ...
        <p className="intro__lead">
          過去10年間の日本株の株価上昇事例（10倍株・7倍株）の傾向調査をもとに定めた
          9つの財務・株価条件を、対象市場の全{screening.counts.population.toLocaleString()}
          銘柄に機械的に適用し、8条件以上（未達は最大1条件まで）を満たした
          {screening.counts.passed.toLocaleString()}
          銘柄を全件掲載しています（うち9条件すべて充足は
          {screening.counts.full_match.toLocaleString()}銘柄）。
        </p>
      </section>
      ...
      <section className="sources" aria-label="データの出所">
        ...
        <ul>
          ...
          <li>株価・出来高・時価総額・上場からの年数の近似値：外部market data提供元</li>
        </ul>
      </section>
